...
<table border="1">
    ...
    <tr>
        <td>作者</td>
        <td>Shiwei Lu, Ruihu Li, Wenbin Liu, Xuan Chen</td>
    </tr>
    <tr>
        <td>来源</td>
        <td>Computers & Security, Volume 121 [CCF 网络与信息安全 B 类期刊]</td>
    </tr>
    <tr>
        <td>年份</td>
        <td>2022 年 10 月</td>
...
3. 我们将一种在深度学习中具有后门神经元激活的检测方法移植到联邦学习中，并改进它以防御早期攻击。与原始检测相比，我们的方法使服务器能够自动检测并从聚合模型中删除嵌入式后门，并具有更好的性能。
...
1. 攻击者不能通过自适应后门攻击来逃避防御。（攻击者只能通过调整 `$\Delta w_j^r$` 去改变 `$cos(w_p^r-w_g^r, w_i^r)$`。）
2. 具有较少的计算开销。（计算局部更新和预聚合更新之间的余弦相似度，因此时间复杂度为 `$O(d·m)$`。）
...
1. 使用 `$N_1$` 个随机模式通过求解优化问题（8）得到它们的神经元表示 `$r(\hat{x}(\delta, m))$` 和 `$r(x)$`；
2. 计算第 `$k$` 个模型的异常分数 `$L_k(k=1,...,m)$` ![公式10](公式10.png)；
3. 计算聚合模型的异常分数；
4. 计算所有异常分数的标准差，记为 `$std(L_1,L_2,...L_m,L_G)$`
...
![早期轮次攻击下局部模型的异常得分](早期轮次攻击下局部模型的异常得分.png)
