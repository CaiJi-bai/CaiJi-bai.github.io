...
（1）基于错误率的拒绝（ERR）：法删除了对全局模型的错误率具有较大负面影响的局部模型（受 RONI 的启发，后者删除了对模型的错误率具有较大负面影响的训练示例）
（2）基于损失函数的拒绝（LFR）：除去了局部模型导致大量损失（受 TRIM 的启发，删除了对损失有较大负面影响的训练示例）。
（3）ERR 和 LFR 结合

![防御结果](防御结果.png)

本实验中考虑部分知识攻击。表 3 的结果说明三方面问题：
一是，LFR 与 ERR 相当或比 ERR 更有效，即 LFR 的测试错误率与 ERR 相似或小得多。
二是，在大多数情况下，Union 与 LFR 相当，但在一种情况下（Krum + LFR 对 Krum 和 Krum + Union 对 Krum），Union 更有效。
第三，LFR 和 Union 可以在某些情况下有效防御本文提出的攻击方法。
