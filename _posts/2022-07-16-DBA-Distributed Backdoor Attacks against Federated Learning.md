...
1. 基于 Kerckhoffs 理论，我们认为这里的**强攻击者完全控制其局部训练过程**，例如后门数据注入和更新局部训练超参数，包括 `$E$` 和 `$l_r$`。
...
+ U 形曲线。这是因为图像的中间部分通常包含主要对象。这些区域的 DBA 很难成功，并且会很快被遗忘，因为这些像素是主要精度的基础。
