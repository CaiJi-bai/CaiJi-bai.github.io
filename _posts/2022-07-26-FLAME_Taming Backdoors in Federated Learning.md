...
其中 `$\varepsilon$` 表示隐私界限，`$\delta$` 表示打破这个界限的概率。较小的 `$\epsilon$` 和 `$\delta$` 值表示更强的隐私。
...
与良性模型的权重向量相比，训练良好的后门模型的权重向量 `$W$` 具有更高的角度差。

FLAME 部署了一种聚类方法来识别此类中毒模型，并将其从FL聚合中删除。

如图 3a 所示，聚类方法旨在删除具有高攻击影响（具有较大角度偏差）的模型，而不是所有恶意模型。

问题：我们需要一种也可以在动态攻击设置中工作的方法，即注入后门的数量是未知的，并且可能在训练轮次之间变化。
