...
2. 对三个自然语言处理任务（Reddit 的下一个单词生成以及 IMDB 和 Sentiment140 的情感分类）、两个模型架构（LSTM 和 Transformer）以及三个计算机视觉数据集（CIFAR-10、CIFAR-100 和 EMNIST 的分类）、两个模型架构（ResNet 和 LeNet）进行了广泛的实验评估，以对抗有防御的 FL 系统。

# 联合学习中的持久后门

![神经毒素与基线对比实验结果](神经毒素与基线对比实验结果.png)

### 威胁模型

1. 攻击者只能破坏 FL 中一小部分设备（<1%）。
2. 受感染的设备可以在 FL 培训课程中参与有限的次数。我们将此参数称为 AttackNum，并在我们的实验中对其进行更改，
...
![Hessian迹和顶部特征值](Hessian迹和顶部特征值.png)
...
1. 提高了攻击的持久性
2. 增加了后门对再训练的鲁棒性
3. 通过使用神经毒素，攻击者可以嵌入由单个单词触发的后门。（后门触发更容易？）