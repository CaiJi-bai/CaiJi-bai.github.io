...
联邦学习可以从数以万计设备上学习一个聚合的模型，这个用户数量 n 是非常非常大的。在第 t 轮，服务端选取 m 个用户组成一个集合 S_m，然后把当前的联合模型 G^t 发给他们。在这个过程中 m 的选择需要权衡效率和学习速度。然后被选中的用户基于 G^t 模型和本地数据训练得到本地模型 L^{t+1}，并且把差值 L{t+1}-Gt 返回给 server。然后更新完新的模型就是：